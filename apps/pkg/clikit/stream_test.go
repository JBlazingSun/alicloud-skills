...
func TestResolveToolResultName(t *testing.T) {
	cases := []struct {
		...
		{name: "event name preferred", evtName: " file_read ", fallback: "bash", want: "file_read"},
		{name: "fallback when empty", evtName: "   ", fallback: "bash", want: "bash"},
		{name: "both empty", evtName: "", fallback: "   ", want: ""},
	}
	for _, tc := range cases {
...
func TestNormalizeSummaryText(t *testing.T) {
	if got := normalizeSummaryText(" a\n\tb\r\n c "); got != "a  b   c" {
		t.Fatalf("unexpected normalized text: %q", got)
	}
	if got := normalizeSummaryText(""); got != "" {
